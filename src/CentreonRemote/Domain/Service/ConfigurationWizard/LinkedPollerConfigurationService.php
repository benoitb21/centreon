...
class LinkedPollerConfigurationService
{
     ...
     *
     * @param string $resourceName the name of the broker configuration
     * @return array the broker configuration template
     */
    private function getResource($resourceName): array
    {
        return require_once __DIR__ . "/../../../{$this->resourcesPath}/{$resourceName}";
    }
    ...
    private function setBrokerInputOfRemoteServer($remoteId, PollerServer $poller): void
    {
        ...
        $brokerInputFlow = $this->getResource('input_flow_one_peer_retention.php');
        $brokerInfosEntities = $brokerInputFlow($poller->getName(), $poller->getIp());
